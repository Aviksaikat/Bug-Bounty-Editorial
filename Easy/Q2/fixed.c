...
PROBLEM STATEMENT
    ...
    1. Calculating the COUNT of each digit that appeared in the string.
...
INSTRUCTIONS
    ...
    3. You are supposed the logic for counting the frequency of characters correctly 
...
OUTPUT: 
    The OUTPUT Printed on your console is the key to enter into website.
    hjjedgkohw - This is the Answer.
*/
#include <stdio.h>
#include <string.h>
#include <math.h>
...
int main() {
    ...
    char *s; //BUG CORRECTED: Should be a pointer
    s = getString();
    int arr[10]={0},a;               //BUG CORRECTED - Should be initialized with 0 and not 1.
    for (int i=0; i< strlen(s); i++) 
    {
        if(isdigit(s[i]))
            {
                a=(int)s[i]-'0';  //BUG CORRECTED - WE SHOULD SUBTRACT '0' and NOT 'O'
                arr[a]++;
            }
    }
    hash(arr);
