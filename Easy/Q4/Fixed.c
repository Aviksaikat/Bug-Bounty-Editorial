...
ANSWER
        ...
        BELORTU
        -CJ
        Password: CYYYENYEHLXEWZERINBELORPU
*/
#include<stdio.h>
#include "helpers.h"
...
void main()
{
    ...
    for(int i=0;mywords[i][0]!='\0';i++)        //BUG CORRECTED - Incorrect Code has mywords[i] only instead of mywords[i][0]
    {
        sort(mywords[i],strlen(mywords[i]));    //BUG CORRECTED - Incorrect Code has &mywords[i] instead of mywords[i]
        printf("%s ",mywords[i]);
    }
    ...
    printf("%c\n",74);
    printf("KEY: ");
    hashmywords(mywords,7);//this function is correct. Do not change it.
}
void sort(char* s,int size)
{
    for(int i=0;i<size-1;i++)
    {
        for(int j=i+1;j<size;j++)
        {
            ...
            {
                int temp=*(s+j);        
                *(s+j)=*(s+i);          //BUG CORRECTED - Incorrect Code has *(s+j)=*(s+j+1) instead of *(s+j)=*(s+i)
                *(s+i)=temp;            //BUG CORRECTED- Incorrect Code has *(s+j+1)=temp instead of *(s+i)
            }
        }
    }
}
